                            ...
                            <div class="card shadow mb-4">
                                ...
                                <div class="card-body">
                                    <div class="chart-area" style="height:500px;">
										...
										$sql="select * from user where (username='$usernamez' or email='$email');"; 
										//Check whether username or email already exist in database or not

										$res=mysqli_query($hubungan,$sql);

										if (mysqli_num_rows($res) > 0) {
												$row = mysqli_fetch_assoc($res);
												if($username==isset($row['username']) && $email==isset($row['email']))
												{
													echo "<center><div class='alert alert-danger'>Sorry, the voter username or email already exists in our database<br> <a href='add_voters.php'> Go Back !</a></div></center>";
												}
										}else{
											//put query to insert record
											$q1 = "insert into user (username, email, password, name, usertype, district)
											values ('".$usernamez."','".$email."','".$passwordz."','".$name."', '".$usertype."','".$district."')"; 
											...
											if(!$resultInsert){
												...
												echo "<div class='alert alert-success'>Voters was inserted successfully.<a href='add_voters.php'> Go Back!</a></div>";
											}
										}
										?>
									</div>
